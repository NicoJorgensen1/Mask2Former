...
def create_batch_img_ytrue_ypred(config, data_split, FLAGS, data_batch=None, model_done_training=False, device="cpu"):
    ...
    for data in data_batch:                                                                 # Iterate over each data sample in the batch from the dataloade
        printAndLog("this is the databatch {}".format(data_batch), logs=FLAGS.log_file)
        printAndLog("This is the data {}".format(data), logs=FLAGS.log_file)
        printAndLog("The data is of type {}".format(type(data)), logs=FLAGS.log_file)
        printAndLog("The first element of data is of type {}".format(type(data[0])), logs=FLAGS.log_file)
        printAndLog("The first element of data contains following keys {}".format(data[0].keys()), logs=FLAGS.log_file)
